# Capítulo 10 — Filas de prioridade / Heaps (código-fonte)
...
- `heap.h` / `.c` — min-heap em array: `sift-up`/`sift-down`, `push`/`pop`,
  `build-heap` $O(n)$.
- `main.c` — demonstração (inclui heapsort); a saída aparece no capítulo.
